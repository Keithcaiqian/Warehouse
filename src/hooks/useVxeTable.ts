...
export default function useVxeTable(params: VxeGridProps, dataChange?: (data: any) => void) {
  ...
  const table = reactive<VxeGridProps>(initConfig as VxeGridProps);
  ...
  const methods = {
    ...
    moveRowTopOrBottom(row: string | {} | {}[] | string[] | number[], type: -1 | 1) {
      ...
      // 没移动或超出边界
      if (
        (type === -1 && targetIndex <= 0) ||
        (type === 1 && targetIndex >= table.data.length - 1)
      ) {
        return false;
      }
    ...
    moveRowByIndexTopOrBottom(indexList: number[], type: -1 | 1) {
      // 没移动或超出边界
      if (
        (type === -1 && indexList[0] <= 0) ||
        (type === 1 && indexList[indexList.length - 1] >= table.data.length - 1)
      ) {
        return false;
      }

      const moveNumber =
        type === -1 ? -indexList[0] : table.data.length - 1 - indexList[indexList.length - 1];
